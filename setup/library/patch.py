...
options:
  ...
  out:
    description:
      - Path of the file on the remote machine where the patch output should go.
      - If specified, the file being patched is not patched in-place, but a new
      - file is created based on the original file and the patch.
    required: false
note:
...
def apply_patch(patch_func, patch_file, basedir, dest_file=None, binary=False, strip=0, dry_run=False, backup=False, out_file=None):
...
def main():
    ...
    if p.out:
        if p.dest and not os.access(p.dest, R_OK): 
            module.fail_json(msg="dest %s doesn't exist or not readable" % (p.dest))
        #if not os.access(p.out, W_OK):
        #    module.fail_json(msg="out %s doesn't exist or not writable" % (p.dest))
    else:
    ...
    changed = False
    if not is_already_applied(patch_func, p.src, p.basedir, dest_file=(p.out or p.dest), binary=p.binary, strip=p.strip):
        try:
            apply_patch( patch_func, p.src, p.basedir, dest_file=p.dest, binary=p.binary, strip=p.strip,
                         dry_run=module.check_mode, backup=p.backup, out_file=p.out )
            changed = True
        except PatchError, e:
